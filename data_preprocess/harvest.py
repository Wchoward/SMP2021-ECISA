...
#             punct_regex = r"[、！？｡＂＃＄％＆＇（）＊＋，－／：；＜＝＞＠［＼］＾＿｀｛｜｝～｟｠｢｣､、〃》「」『』【】〔〕〖〗〘〙〚〛〜〝〞〟〰〾〿–—‘’‛“”„‟…‧﹏!\"\#$%&\'\(\)\*\+,-\./:;<=>?@\[\\\]\^_`{\|}~]"
#             matched = re.search(punct_regex, entity_type,
#                                 re.MULTILINE | re.UNICODE)
#             if matched:
#                 punct0 = matched.group()
#                 raise Exception("Your type input '{}' includes punctuation '{}', please remove them first".format(
#                     entity_type, punct0))
...
#             entity_type_dict = {
#                 entity: "添加词" for entity in self.entity_mention_dict}
...
#                     # 不允许同一实体有不同类型
#                     warnings.warn(
#                         "You've added an entity twice with different types, the later type will be used.")
...
#                             _visit(_trie[ch], _word[1:],
#                                    _tolerance - 1, _mention+ch)
#             else:
#                 if 'leaf' in _trie:
...
#                     pinyin_cands |= {
#                         tuple(tmp[:i] + [neibr] + tmp[i + 1:]) for neibr in self.pinyin_adjlist[pinyin]}
...
#                     mention_cands += self.get_pinyin_correct_candidates(
#                         word, pinyin_tolerance)
#                 if char_tolerance is not None:
#                     mention_cands += self.search_word_trie(
#                         word, char_tolerance)

#                 if len(mention_cands) > 0:
#                     entity0, type0 = self.choose_from_multi_mentions(
#                         mention_cands, sent)
#                 if entity0:
#                     l = sent.find(word)
...
#                         overlap_surface_entity_with_pos[surface0] = (
#                             [l, r], entity_type0)
#                         for ll in range(l + 1, r):
#                             rr, entity_types_2 = self.dig_trie(sent, ll)
#                             if rr != -1 and rr <= len(sent):
#                                 surface0_2 = sent[ll:rr]  # 字面值
#                                 entity_type0_2 = self.choose_from(
#                                     surface0_2, entity_types_2)
#                                 overlap_surface_entity_with_pos[surface0_2] = (
#                                     [ll, rr], entity_type0_2)
#                         # 再利用频率比较这些映射
#                         candidate, cnt_cand = None, 0
...
#                             if i == 0 or cnt0 > cnt_cand:
#                                 candidate, cnt_cand = (
#                                     [ll, rr], entity_type00), cnt0
#                         entities_info.append(candidate)
#                         l = candidate[0][1]
#                     else:
#                         # 字典树能根据键找到实体范围，选择则依然需要根据历史等优化
#                         entities_info.append(([l, r], entity_type0))
#                         l = r
#                 else:
#                     # 字典树能根据键找到实体范围，选择则依然需要根据历史等优化
#                     entities_info.append(([l, r], entity_types))
#                     l = r
#             else:
#                 l += 1
...
#         '''
#         self.check_prepared()
#         entities_info = self._entity_linking(
#             sent, pinyin_tolerance, char_tolerance, keep_all)
#         if (not keep_all) and (pinyin_tolerance is not None or char_tolerance is not None):
#             self._entity_recheck(sent, entities_info,
#                                  pinyin_tolerance, char_tolerance)
#         if with_ch_pos:
#             ch_pos = []
...
#         self.check_prepared()
#         entities_info = self._entity_linking(
#             sent, pinyin_tolerance, char_tolerance)
#         sent2 = self.decoref(sent, entities_info)
#         l = 0
#         i = 0
#         for word, flag in pseg.cut(sent2):
...
#             if entity_type_dict is None:
#                 entity_type_dict = {
#                     entity: "添加词" for entity in entity_mention_dict}
...
#             if drop_empty_line:
#                 sentences = [
#                     sent for sent in sentences if len(sent.strip()) > 0]
#             return sentences
